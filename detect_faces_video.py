...
vs = VideoStream(src=0).start()    #index 0 - generally this would be the laptop's built-in camera or desktop's first camera detected
# use FileVideoStream class for parsing video file
time.sleep(2.0)

#loop over the frames from video stream
while True:
    # grab the frame from the threaded video stream and resize it to have maximum width of 400 pixels
    frame = vs.read()
